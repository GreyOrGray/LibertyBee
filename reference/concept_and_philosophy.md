...
**Status:** Canon — drafted 2026-06-08 from the project owner's answers + existing design notes.
**Scope:** The *rationale* behind Liberty Bee's design — why each mechanism exists and what it's for. **Mechanics** (rates, schedules, tables) live in the system reference and the detailed technical references; **current rules** live in the business-rules reference. This doc is the layer that lets a reviewer judge whether a proposed change *aligns with intent*.

> This doc states *intent*; current rules and values live in the business-rules reference. Liberty Bee remains an evolving research project, but the headline figures referenced here are drawn from the frozen, reproducible 1.0 baseline.
...
- Every parameter that grounds a decision carries a **cited source** in the evidence base ("says who?"), so anyone can re-calibrate to their own market and see exactly what our defaults assume.
...
> **TCS is explicitly NOT an ownership or equity claim.** A settled design decision *removed home-purchase as a core promise* of the model — TCS is a non-cash housing-stability credit, not ownership savings. Ownership pathways (incl. the "downpayment" angle in the cap rationale below) remain *possible future programs*, not part of the current model.
...
> ⚠️ **V1 reality:** this 4-tier ladder is *intent*. The V1 engine wires only self-cure + deterministic eviction at 3 consecutive misses — no payment plans, no structured intervention, no relief-before-eviction, and no post-filing cure. The ladder is the target, not the current behavior.
...
A companion principle: **the market acts independently of Liberty Bee's ability to purchase** — properties list, sell to others, and move regardless of LB's financial state. LB competes in a market that doesn't wait for it; the model never lets LB's constraints conveniently slow the world down. *(A core design principle.)*
...
*Source: project owner answers (2026-06-08); the project's design notes and system reference.*
