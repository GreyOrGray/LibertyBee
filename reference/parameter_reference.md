...
**Audience:** a developer tuning or auditing the model. Every behavioral number lives in `reference.ParameterRegistry` as a `(Category, Name, ProjectionID)` row; this is the catalog. The four basis labels — **CITED / DECLARED / MECHANICAL / POLICY** — are the honesty machinery, and the most important column here. Citations behind the CITED knobs live in the evidence base.

> **Note:** Values are the live global (`ProjectionID IS NULL`) defaults. Per-projection *overrides* (stress projections, capital-ladder rungs) exist for some knobs and are noted where relevant but not enumerated. Knobs with no live reader are flagged **⚠ dormant**.
...
|---|---|---|---|---|
| INF | 66 | Inflation-regime Markov model | inflation_engine | DECLARED (anchored to CITED episodes) |
| MARKET | 60 | Property-market state machine | property_market_manager | CITED (FRED) + DECLARED |
| INC | 34 | Income model | income_model / inflation_engine | CITED/CALIBRATED |
...
| GRT | 12 | Grants — **reserved, unimplemented** | none (⚠ reserved) | N/A |
| LEASE | 9 | Lease lifecycle | lease_renewal / eviction | POLICY |
| RET | 7 | Tenant retention model | retention_model / lease_renewal | DECLARED (anchored §10) |
...
| Knob | Default | Basis | Meaning |
|---|---|---|---|
| CashFloorMonths | 3 | **CITED** | Unrestricted-Cash floor = 3 mo OpEx (NORI nonprofit-reserve minimum, evidence §6). |
| EIPAllocationPct | 0.05 | POLICY (design intent) | Intended 5%-of-starting-funds EIP envelope. **⚠ ORPHANED** — not read anywhere (not even loaded into `ProjectionConfig`). |
| EIPStartYear | 6 | POLICY (design intent) | Year EIP spend would unlock. **⚠ ORPHANED** — same as above. |

*(`FIN.StartingFunds` (starting capital) is a **per-projection** knob — it's what defines each funding rung, so it isn't in the global 3.)*

## CSF (5) — reserve curve · reader: `fund_manager` / `property_acquisition_manager`
| Knob | Default | Basis | Meaning |
|---|---|---|---|
...
| ReserveCurveN0Properties | 8 | **DECLARED** | Portfolio size holding the peak; taper `floor+(peak−floor)·√(N0/N)` begins above it. A declared decision, swept 8/12/15. |
...
| VacancyRateBase | 0.019 | **CITED** | Salem rental vacancy 1.9% (City Housing Roadmap, evidence §4a). Steady-state only — cyclical stress is the inflation-regime model's job. *(Note: earlier docs said "acquisition scoring only", but it also feeds tenant_manager fill-duration.)* |
...
| PreScreenRentToIncomeRatio | 0.40 | DECLARED | Self-selection threshold — households apply only if rent ≤ 40% income (models applicant self-filtering). |
...
| MaxOccupantsStudio | 2 | **CITED** | Studio (0BR) cap (own knob; age-aware revisit is a future refinement). |
...
| LateFeePercent | 5.0 | POLICY | Late fee % of rent (LATE months only). *(Note: LATE is structurally never written in V1.)* |
...
| EarlyBreakProbMonthly | 0.0025 | Monthly voluntary early-break hazard (day-1 gated; a prior daily-roll bug inflated it ~19×, since fixed). |
...
## RET (7) — tenant retention model · reader: `retention_model` (wired via `lease_renewal_manager`) · **DECLARED, anchored to CITED evidence §10**
...
| ScarcitySensitivityGamma | 0.50 | DECLARED, swept 0.25–0.75 (Sieg-Yoon lock-in §10b/c) | γ — how strongly regional scarcity suppresses exit; scarcity ≈28% of retention at this value. |
...
| MoverRegionalVacancyPct | 0.030 | DECLARED, secondary-sourced (North Shore/Essex ≈ACS5yr, §4a) | Regional vacancy a leaving tenant faces — separate from `PROP.VacancyRateBase` (LB's Salem-local occupancy). |
...
| Knob | Default | Basis | Meaning |
|---|---|---|---|
| CreditCap | $15,000 | POLICY | Cap on current balance (refillable). |
...
*(`TCS.CreditRate` (credit accrual rate, e.g. 10%) is set **per projection**, not globally — so it isn't in the global 20; see the projection definitions.)*

## INC (34) — income model · readers: `income_model`, `inflation_engine` (wage pass) · **CITED/CALIBRATED to evidence §9**
...
| &lt;Regime&gt;_WageMean / _WageVol (10) | Normal 3.1%/0.8% · Surge 5.0%/1.2% · Nzn 3.5%/0.8% · DF 0.0%/1.0% · DS 1.5%/1.5% | **CALIBRATED** (anchor CITED EPI) | Per-regime wage growth; tuned as a set so E[rent−wage] = +0.41 pp/yr (inside the ratified 0.3–0.5 floor, evidence §9c). A first draft at +0.10 was rejected as survival-flattering. |

## TNT (16) — applicant/household generation · reader: `tenant_manager`
| Knob(s) | Default | Basis | Meaning |
|---|---|---|---|
| HouseholdType{Single,Couple,Family,Roommates}Weight | 35/30/25/10 | **CITED** | Household-type mix — re-leveled to Salem renter shape (~35% single; evidence §4c). |
...
| RaiseTier{Top,Mid,Low}CushionMonths | 12/10/8 | POLICY | Cash-cushion thresholds for the raise tiers. |
...
`TurnoverDuration{Inspect,Clean,Paint,FinalInspect}` = 2/3/5/1 days; `Restoration{Min,Max}{Eviction,Voluntary}` = Eviction 15–30, Voluntary 10–30 days (eviction always forces restoration — "a wound", POLICY-flavored). *(`MaxApplicationsPerVacancyPerWeek` was loaded-but-never-enforced — since dropped.)*

## GRT (12) — grants · **⚠ RESERVED / UNIMPLEMENTED — no live reader for any of the 12**
`Chance{Small,Medium,Large}` = 0.20/0.10/0.05; `{Min,Max}Grant{Small,Medium,Large}` = Small $10k–50k, Medium $50k–150k, Large $150k–500k; `Cooldown{Small,Medium,Large}` = 12/24/36 mo. Schema-only design placeholders; `grant_manager.py` does not exist. See the `GrantLedger` table.

## INF (66) — inflation-regime Markov model · reader: `inflation_engine` · **DECLARED, anchored to CITED episodes** (evidence §7)
**Standalone:** `Mode`=Regime (default), `StartRegime`=Normal.
...
*Basis labels are this reference's synthesis; the underlying citations live in the evidence base.*
