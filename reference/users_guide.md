...
- [`engine_internals.md`](engine_internals.md) — the turn-by-turn engine (what runs, in what order), the reproducibility model, and a per-module reference for all 27 modules.
- [`data_dictionary.md`](data_dictionary.md) — every one of the 48 tables: purpose, keys, owner, readers.
- [`parameter_reference.md`](parameter_reference.md) — every one of the 328 knobs, with its basis label and reader.

Other companions: [`concept_and_philosophy.md`](concept_and_philosophy.md) (the why), [`business_rules.md`](business_rules.md) (the exact rules), [`evidence_base.md`](evidence_base.md) (where every calibrated number comes from), [`architecture_overview.md`](architecture_overview.md) (the code map), [`failure_modes.md`](failure_modes.md) (how the model dies — every failure path deliberately forced and verified, with recipes to do it yourself).
...
When a unit is vacant, `tenant_manager` generates a **slate** of applicant households for it (default 10 per fill-attempt), and this is where the model's realism lives. Each applicant is built in three draws: a **household type** (single / couple / family / roommates, from the Salem-renter-calibrated mix), a **composition** (adult/child counts), and — the important part — an **income**. Income is not a single number pulled from a band; it's the **sum of independently-drawn earners** (`income_model`): a couple is a primary earner plus a second earner who *may or may not* work (with realistic assortative-mating correlation), a single might be a fixed-income retiree, and every earner's dollars **grow over the 20 years on the same economic-weather path as rent** (so incomes and rents scissor apart honestly instead of one being frozen). The whole slate is seeded deterministically per `(vacancy, date)`, so the same run always draws the same applicants.
...
>
> This section is the **tuner's shortlist** — the knobs you'd actually reach for, grouped by what you touch first. The **complete catalog of all 328 knobs** (every one with its basis label and reader module) is [`parameter_reference.md`](parameter_reference.md).
...
*(Tier slots 4 and 5 are absent-reserved — to add a deeper tier, seed both an `RR.<Nth>ReductionMonths` and `...Pct` row. The historical 4th-tier seeding artifact has since been retired.)*

### 2.3 The reserve curve
...
### 2.4 The economic weather — 5 regimes × 7 numbers + a transition matrix
...
python app/src/simulation.py --env <db> --projection-id 206 --months 240 --seed 12345

# 3. Verify it reproduces + the published numbers hold (the shipped verification tools)
python reproduction_gate.py --corpus <restored corpus DB name>   # every sampled run reproduces from the engine, to the penny
python site_metrics.py --corpus <restored corpus DB name>        # every site figure recomputed vs its published value
...
A **projection** is a named scenario: a starting-capital level plus any per-projection knob overrides. The funding ladder spans **$2M–$11M**: IDs **200–209** cover $5–11M, and the synthetic low-funding rungs **300–305** (defined by `reproduce_rungs.sql`, applied automatically by `regenerate_corpus.py`) cover $2–4.5M; **220–222** are stress scenarios. Survival statistics come from sweeping many seeds per projection.
...
Stated plainly, because an honest model names its own edges. None of these affect the frozen 1.0 survival numbers unless noted; the mission-tension items are tracked openly as known divergences.

- **Tenants don't have lives yet.** Hardship is a monthly probability, not a story; there are no second-order relations, no life events, no aging (ages are cosmetic, and a generated family can even have mismatched surnames). Deep per-tenant lives are the V0.4 tenant engine — the single biggest realism gap.
- **Two eviction/deposit mechanics fight the mission and are flagged, not hidden:** eviction has **no cure period** (paying up during the 2-month window doesn't stop execution), and a partially-funded installment deposit forfeits money already paid in on any early exit. Both are documented tensions awaiting the tenant-engine era.
- **The rent/market model is coarse:** market rent is `f(bedrooms)` plus a bathroom bump — no square footage, location, or condition. And in 1.0 the engine charges tenants off `BaseRent` while acquisition underwrites off the bathroom-adjusted `AdjustedRent`, with a fractional-bath overpricing bug — both **acquisition-scoring-only in 1.0** (no tenant rent, no published number moves) and both fixed in V2.
...
- **Vacancy is steady-state.** Cyclical soft-market stress enters *only* through the inflation regimes' vacancy deltas — there's no independent vacancy-shock model. Don't widen `PROP.VacancyFluctuationBand` to fake a downturn.

*Guide vintage: **release 1.0** (**re-baselined and frozen** as the **1.0** baseline of record (corpus V03R4, engine **0.5.0**), 2026-07-06). Registry snapshot: **328 global knobs** (up from 292; the most recent additions are the sum-of-earners income model, which retired the static `reference.IncomeBand` table + `TNT.IncomeBand*Weight`). MARKET/ACQ/CMPL knob sections land in the v2 build-out.*
