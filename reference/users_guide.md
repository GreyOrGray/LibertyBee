...
> All knobs are rows in the parameter store — a global default in `reference.ParameterRegistryDefault`, an optional per-projection override in `reference.ParameterRegistryDefined` (split at V00071). Reads are **fail-loud**: a missing knob crashes rather than silently defaulting. Change values with an UPDATE (experiments) or a migration (permanent).
>
