-- GENERATED by corpus_schema_translate.py from corpus_schema.sql —
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.run_summary (
        RunSummaryID    INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung            DECIMAL(18,2)   NOT NULL,   -- $5000000.00 .. $7500000.00
        Seed            INT             NOT NULL,
        ProjectionID    INT             NOT NULL,   -- 200..205
        EngineVersion   citext    NOT NULL,   -- "1.0.0" for Phase 3.10.6
        EphemeralDBName citext   NOT NULL,   -- audit trail
        StartedAtUtc    TIMESTAMP       NOT NULL,
        CompletedAtUtc  TIMESTAMP       NOT NULL,
        FinalCash       DECIMAL(18,2)   NOT NULL,
        FinalCSF        DECIMAL(18,2)   NOT NULL,
        FinalEIP        DECIMAL(18,2)   NOT NULL,
        FinalCashCSF DECIMAL(18,2) GENERATED ALWAYS AS (FinalCash + FinalCSF) STORED,
        FinalTotal DECIMAL(18,2) GENERATED ALWAYS AS (FinalCash + FinalCSF + FinalEIP) STORED,
        Survived        SMALLINT             NOT NULL,   -- 3.9.6A criterion: FinalCash + FinalCSF > 0
        ...
        AvgHouseholdSize DECIMAL(21,15) GENERATED ALWAYS AS (CASE WHEN HouseholdCount > 0 THEN CAST(PersonCount AS DECIMAL(10,4)) / HouseholdCount ELSE 0 END) STORED,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_run_summary_Rung ON v1.run_summary (Rung);
    CREATE INDEX IF NOT EXISTS IX_v1_run_summary_Survived ON v1.run_summary (Survived);

-- ---------------------------------------------------------------------------
-- v1.fund_ledger — FundLedger rows tagged (Rung, Seed)
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.fund_ledger (
        FundLedgerID    BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_fund_ledger_RungSeedDate ON v1.fund_ledger (Rung, Seed, LedgerDate DESC, EventID DESC);

-- ---------------------------------------------------------------------------
-- v1.tcs_ledger — TenantCreditLedger rows tagged (Rung, Seed)
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.tcs_ledger (
        TCSLedgerID         BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                DECIMAL(18,2)   NOT NULL,
        Seed                INT             NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_tcs_ledger_RungSeedType ON v1.tcs_ledger (Rung, Seed, TransactionType);

-- ---------------------------------------------------------------------------
-- v1.lease — Lease rows tagged (Rung, Seed)
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.lease (
        LeaseRowID                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                        DECIMAL(18,2)   NOT NULL,
        Seed                        INT             NOT NULL,
        ...
        TerminationReason           citext   NULL,
        ConsecutiveMissedPayments   INT             NOT NULL,
        RenewalDecision             citext    NULL,
        CumulativeRentReductionPct  DECIMAL(18,4)   NOT NULL,
        EffectiveMonthlyRent        DECIMAL(18,2)   NOT NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_lease_RungSeed ON v1.lease (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_lease_RungSeedHousehold ON v1.lease (Rung, Seed, HouseholdID);

-- ---------------------------------------------------------------------------
-- v1.lease_termination — LeaseTermination rows tagged (Rung, Seed)
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.lease_termination (
        LeaseTerminationRowID       BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                        DECIMAL(18,2)   NOT NULL,
        Seed                        INT             NOT NULL,
        LeaseID                     INT             NOT NULL,
        ...
        DepositForfeited            SMALLINT             NOT NULL,
        EarlyBreakPenalty           DECIMAL(18,2)   NULL,
        EvictionFiledDate           DATE            NULL,
        EvictionExecutionDate       DATE            NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_lease_termination_RungSeedType ON v1.lease_termination (Rung, Seed, TerminationType);

-- ---------------------------------------------------------------------------
-- v1.lease_termination_ledger — LeaseTerminationLedger rows tagged (Rung, Seed)
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.lease_termination_ledger (
        LTLedgerRowID   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung            DECIMAL(18,2)   NOT NULL,
        Seed            INT             NOT NULL,
        ...
        Metadata        citext   NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_lease_termination_ledger_RungSeedTxn ON v1.lease_termination_ledger (Rung, Seed, TxnType);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.household (
        HouseholdRowID              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                        DECIMAL(18,2)   NOT NULL,
        Seed                        INT             NOT NULL,
        HouseholdID                 INT             NOT NULL,
        ...
        PersonCount INT GENERATED ALWAYS AS (AdultCount + ChildCount) STORED,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_household_RungSeed ON v1.household (Rung, Seed);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.properties (
        PropertyRowID           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                    DECIMAL(18,2)   NOT NULL,
        Seed                    INT             NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_properties_RungSeed ON v1.properties (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_properties_RungSeedAcqDate ON v1.properties (Rung, Seed, AcquisitionDate);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.property_units (
        UnitRowID       BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung            DECIMAL(18,2)   NOT NULL,
        Seed            INT             NOT NULL,
        UnitID          INT             NOT NULL,
        PropertyID      INT             NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_property_units_RungSeed ON v1.property_units (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_property_units_RungSeedProperty ON v1.property_units (Rung, Seed, PropertyID);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.projection_parameters (
        ProjectionID                INT             NOT NULL PRIMARY KEY,
        ProjectionName              citext   NOT NULL,
        StartingFunds               DECIMAL(18,2)   NOT NULL,
        BelowMarketRentPct          DECIMAL(18,4)   NOT NULL,
        RentInflationRate           DECIMAL(18,4)   NOT NULL,
        InflationMode               citext    NOT NULL,  -- Phase 1.6: leg (Regime|Static); replaces the retired InflationScenarioType
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.inflation_schedule (
        InflationScheduleRowID  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                    DECIMAL(18,2)   NOT NULL,
        Seed                    INT             NOT NULL,
        ...
        ScenarioPhase           citext    NULL,
        Notes                   citext   NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_inflation_schedule_RungSeedMonth ON v1.inflation_schedule (Rung, Seed, MonthIndex);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.event_summary (
        EventSummaryID  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung            DECIMAL(18,2)   NOT NULL,
        Seed            INT             NOT NULL,
        ...
        EventType       citext    NULL,
        EntityType      citext    NULL,
        ActionType      citext    NULL,
        EventCount      INT             NOT NULL,
        TotalAmount     DECIMAL(18,2)   NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_event_summary_RungSeed ON v1.event_summary (Rung, Seed);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.monthly_payment_status (
        MonthlyPaymentStatusID BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung                   DECIMAL(18,2) NOT NULL,
        Seed                   INT           NOT NULL,
        LeaseID                INT           NOT NULL,
        MonthIndex             INT           NOT NULL,        -- -1 = sentinel; 1+ = real sim month
        PaymentStatus          citext  NOT NULL,        -- ON_TIME / LATE / MISSED / REDEEMED / 'ALL_ON_TIME'
        DaysLate               INT           NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_monthly_payment_status_RungSeedLease ON v1.monthly_payment_status (Rung, Seed, LeaseID);
    CREATE INDEX IF NOT EXISTS IX_v1_monthly_payment_status_StatusMonth ON v1.monthly_payment_status (Rung, Seed, MonthIndex, PaymentStatus);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.employees (
        EmployeeRecordID BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung             DECIMAL(18,2)  NOT NULL,
        Seed             INT            NOT NULL,
        EmployeeID       INT            NOT NULL,
        RoleID           INT            NOT NULL,
        RoleName         citext  NOT NULL,  -- denormalized from reference.EmployeeRole
        ...
        IsActive         SMALLINT            NOT NULL   -- always 1 in V1
    );
    CREATE INDEX IF NOT EXISTS IX_v1_employees_RungSeed ON v1.employees (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_employees_HiredDate ON v1.employees (Rung, Seed, HiredDate);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.payroll (
        PayrollRecordID BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung            DECIMAL(18,2)  NOT NULL,
        Seed            INT            NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_payroll_RungSeed ON v1.payroll (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_payroll_Date ON v1.payroll (Rung, Seed, PayrollDate);
    CREATE INDEX IF NOT EXISTS IX_v1_payroll_Employee ON v1.payroll (Rung, Seed, EmployeeID);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.compliance (
        ComplianceRowID  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Rung             DECIMAL(18,2) NOT NULL,
        Seed             INT           NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS IX_v1_compliance_RungSeed ON v1.compliance (Rung, Seed);
    CREATE INDEX IF NOT EXISTS IX_v1_compliance_WorkType ON v1.compliance (Rung, Seed, WorkType);
...
-- ---------------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS v1.corpus_meta (
        MetaID         INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        Scenario       citext   NOT NULL,
        SweepMode      citext   NOT NULL,
        EngineVersion  citext   NOT NULL,
        HarnessCommit  citext   NULL,      -- NULL when not run from a git checkout
        HarnessDirty   SMALLINT            NOT NULL,  -- 1 = generated from a modified tree
        HarnessRoot    citext  NULL,
        HostName       citext  NULL,
        StartedUTC     TIMESTAMP(0)   NOT NULL
    );
    CREATE INDEX IF NOT EXISTS IX_v1_corpus_meta_Scenario ON v1.corpus_meta (Scenario);
